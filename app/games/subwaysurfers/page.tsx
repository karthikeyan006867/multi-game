...
export default function SubwaySurfersGame() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('subwaysurfers_high_score');
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      ctx.fillStyle = '#fff';
      ctx.font = '24px Arial';
      ctx.fillText('Subway Surfers', canvas.width / 2 - 80, canvas.height / 2);
  ...
  const handleGameOver = () => {
    ...
    if (score > highScore) {
      setHighScore(score);
      localStorage.setItem('subwaysurfers_high_score', score.toString());
      confetti({
        particleCount: 100,
        spread: 70,
        origin: { y: 0.6 }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-950 via-gray-950 to-black p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-teal-400 to-teal-600 text-transparent bg-clip-text">
            🚇 Subway Surfers
          </h1>
        ...
        <div className="flex justify-center gap-3">
          {!gameStarted ? (
            <motion.button
              onClick={startGame}
              className="bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-700 text-white font-bold py-4 px-8 rounded-xl flex items-center gap-2 shadow-lg"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              <FaPlay />
        ...
        <div className="mt-6 bg-white/10 backdrop-blur-md rounded-xl p-6 border border-white/20">
          ...
          <p className="text-gray-300">
            Subway Surfers is an exciting game! Use arrow keys or controls to play.
        ...
        {gameOver && (
          ...
          >
            ...
            >
              <h2 className="text-4xl font-bold mb-4 text-center bg-gradient-to-r from-teal-400 to-teal-600 text-transparent bg-clip-text">
                Game Over!
              </h2>
              ...
              <div className="flex gap-3">
                <motion.button
                  onClick={startGame}
                  className="flex-1 bg-gradient-to-r from-teal-500 to-teal-600 text-white font-bold py-3 rounded-xl"
                  whileHover={{ scale: 1.05 }}
                >
                  Play Again
                </motion.button>
